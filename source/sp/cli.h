...
typedef struct spn_cli_usage spn_cli_usage_t;

struct spn_cli_usage {
  const c8* name;
  const c8* usage;
  const c8* summary;
  spn_cli_opt_usage_t opts [SPN_CLI_MAX_OPTS];
  spn_cli_arg_usage_t args [SPN_CLI_MAX_ARGS];
  spn_cli_usage_t* commands;
