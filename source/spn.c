...
/////////
typedef enum {
  SP_OS_LINK_MODE_HARD,
  SP_OS_LINK_MODE_SYMBOLIC,
  SP_OS_LINK_MODE_COPY,
} sp_os_link_mode_t;

spn_err_t sp_os_link(sp_str_t source, sp_str_t target, sp_os_link_mode_t mode);
...
void spn_cli_link(spn_cli_t* cli);
void spn_cli_tool(spn_cli_t* cli);
void spn_cli_run_bin(spn_cli_t* cli);
...
spn_err_t sp_os_link(sp_str_t source, sp_str_t target, sp_os_link_mode_t mode) {
  switch (mode) {
    case SP_OS_LINK_MODE_HARD: {
#ifdef SP_POSIX
      if (link(source.buf, target.buf) == 0) {
        return SPN_OK;
      }
      SP_LOG_ERROR("Failed to create hardlink from {} to {}: {}",
        SP_FMT_STR(source), SP_FMT_STR(target), SP_FMT_CSTR(strerror(errno)));
      return SPN_ERROR;
#else
      SP_LOG_ERROR("Hardlinks not supported on this platform");
      return SPN_ERROR;
#endif
    }
    case SP_OS_LINK_MODE_SYMBOLIC: {
#ifdef SP_POSIX
      if (symlink(source.buf, target.buf) == 0) {
        return SPN_OK;
      }
      SP_LOG_ERROR("Failed to create symlink from {} to {}: {}",
        SP_FMT_STR(source), SP_FMT_STR(target), SP_FMT_CSTR(strerror(errno)));
      return SPN_ERROR;
#else
      SP_LOG_ERROR("Symlinks not supported on this platform");
      return SPN_ERROR;
#endif
    }
    case SP_OS_LINK_MODE_COPY: {
      sp_err_t err = sp_os_copy_file(source, target);
      if (err != SP_OK) {
        SP_LOG_ERROR("Failed to copy file from {} to {}",
          SP_FMT_STR(source), SP_FMT_STR(target));
        return SPN_ERROR;
      }
      return SPN_OK;
    }
...
void spn_app_init(spn_app_t* app, u32 num_args, const c8** args) {
  ...
  const c8* const usages [] = {
    ...
    "  list           List all project dependencies and versions\n"
    "  link <package> Link package binaries or libraries to project\n"
    ...
    "  tool           Manage global tool installations\n"
    "  run <bin>      Compile and run a binary with TCC",
    SP_NULLPTR
  };
...
void spn_cli_run(spn_app_t* app) {
  ...
  else if (sp_cstr_equal("link", cli->args[0])) {
    spn_cli_link(cli);
  }
  else if (sp_cstr_equal("tool", cli->args[0])) {
    spn_cli_tool(cli);
  }
  else if (sp_cstr_equal("run", cli->args[0])) {
    spn_cli_run_bin(cli);
  }
}
...
void spn_cli_list(spn_cli_t* cli) {
  ...
  sp_ht_for(app.deps, it) {
    ...
    sp_str_builder_append_fmt(
      ...
      SP_FMT_STR(spn_semver_to_str(dep->metadata.version))
    );
    sp_str_builder_new_line(&builder);
  }
...
void spn_cli_link(spn_cli_t* cli) {
  bool link_bin = false;
  bool link_lib = false;
  const c8* mode_str = "hard";
  const c8* package_name = SP_NULLPTR;
  ...
  argparse_init(
    ...
    (struct argparse_option []) {
      OPT_HELP(),
      OPT_BOOLEAN(0, "bin", &link_bin, "link binaries", SP_NULLPTR),
      OPT_BOOLEAN(0, "lib", &link_lib, "link libraries", SP_NULLPTR),
      OPT_STRING(0, "mode", &mode_str, "link mode: hard, symbolic, or copy", SP_NULLPTR),
      OPT_END()
    },
    (const c8* const []) {
      "spn link <package> [--bin] [--lib] [--mode (hard|symbolic|copy)]",
      "Link package binaries or libraries to project",
      SP_NULLPTR
    },
    SP_NULL
  );
  ...
  if (cli->num_args < 1) {
    SP_LOG_ERROR("Package name required");
    return;
  }

  package_name = cli->args[0];

  // Default to bin if neither specified
  if (!link_bin && !link_lib) {
    link_bin = true;
  }

  // Parse link mode
  sp_os_link_mode_t link_mode = SP_OS_LINK_MODE_HARD;
  if (sp_cstr_equal(mode_str, "symbolic")) {
    link_mode = SP_OS_LINK_MODE_SYMBOLIC;
  } else if (sp_cstr_equal(mode_str, "copy")) {
    link_mode = SP_OS_LINK_MODE_COPY;
  }

  // Find the dependency
  sp_str_t name = sp_str_view(package_name);
  spn_dep_context_t* dep = sp_ht_getp(app.deps, name);
  if (!dep) {
    SP_LOG_ERROR("Package {} not found in dependencies", SP_FMT_STR(name));
    return;
  }

  // Link binaries
  if (link_bin) {
    sp_dyn_array_for(dep->package->toml.bin, i) {
      toml_table_t* bin_table = toml_array_table(dep->package->toml.bin, i);
      sp_str_t bin_name = spn_toml_str(bin_table, "name");

      sp_str_t source = sp_os_join_path(app.paths.store, SP_LIT("bin"));
      source = sp_os_join_path(source, bin_name);

      sp_str_t target = sp_os_join_path(app.paths.project.dir, bin_name);

      if (sp_os_link(source, target, link_mode) == SPN_OK) {
        SP_LOG("Linked binary {} from {}", SP_FMT_STR(bin_name), SP_FMT_STR(source));
      }
    }
  }
  ...
  if (link_lib) {
    ...
    sp_dyn_array_for(entries, i) {
      sp_os_dir_entry_t* entry = entries + i;
      sp_str_t file_name = sp_os_extract_file_name(entry->file_path);

      sp_str_t source = entry->file_path;
      sp_str_t target = sp_os_join_path(app.paths.project.dir, file_name);

      if (sp_os_link(source, target, link_mode) == SPN_OK) {
        SP_LOG("Linked library {} from {}", SP_FMT_STR(file_name), SP_FMT_STR(source));
      }
    }
  }
}
...
void spn_cli_tool(spn_cli_t* cli) {
  if (cli->num_args < 2) {
    SP_LOG_ERROR("Usage: spn tool <subcommand>");
    SP_LOG_ERROR("Subcommands: install, list, run, upgrade");
    return;
  }

  const c8* subcommand = cli->args[1];

  if (sp_cstr_equal("install", subcommand)) {
    // spn tool install foo OR spn tool install .
    if (cli->num_args < 3) {
      SP_LOG_ERROR("Usage: spn tool install <package|.>");
      return;
    }

    const c8* package = cli->args[2];
    sp_str_t tools_dir = sp_os_join_path(sp_os_get_data_path(), SP_LIT("spn/tools"));
    sp_str_t tools_toml = sp_os_join_path(tools_dir, SP_LIT("spn.toml"));

    // Create tools directory if it doesn't exist
    sp_os_create_directory(tools_dir);

    // For now, just print a placeholder message
    SP_LOG("Installing tool: {}", SP_FMT_CSTR(package));
    SP_LOG("Tools will be installed to ~/.local/bin");
    SP_LOG("Tool manifest: {}", SP_FMT_STR(tools_toml));
    SP_LOG_ERROR("Tool install not fully implemented yet");
  }
  else if (sp_cstr_equal("list", subcommand)) {
    // List tools from ~/.local/share/spn/tools/spn.toml
    sp_str_t tools_dir = sp_os_join_path(sp_os_get_data_path(), SP_LIT("spn/tools"));
    sp_str_t tools_toml = sp_os_join_path(tools_dir, SP_LIT("spn.toml"));

    if (!sp_os_is_regular_file(tools_toml)) {
      SP_LOG("No tools installed");
      return;
    }

    SP_LOG("Installed tools from: {}", SP_FMT_STR(tools_toml));
    SP_LOG_ERROR("Tool list not fully implemented yet");
  }
  else if (sp_cstr_equal("run", subcommand)) {
    // spn tool run $package - run first binary from package
    if (cli->num_args < 3) {
      SP_LOG_ERROR("Usage: spn tool run <package> [args...]");
      return;
    }

    const c8* package = cli->args[2];
    SP_LOG("Running tool: {}", SP_FMT_CSTR(package));
    SP_LOG_ERROR("Tool run not fully implemented yet");
  }
  else if (sp_cstr_equal("upgrade", subcommand)) {
    // spn tool upgrade $package
    if (cli->num_args < 3) {
      SP_LOG_ERROR("Usage: spn tool upgrade <package>");
      return;
    }

    const c8* package = cli->args[2];
    SP_LOG("Upgrading tool: {}", SP_FMT_CSTR(package));
    SP_LOG_ERROR("Tool upgrade not fully implemented yet");
  }
  else {
    SP_LOG_ERROR("Unknown subcommand: {}", SP_FMT_CSTR(subcommand));
    SP_LOG_ERROR("Available subcommands: install, list, run, upgrade");
  }
}

void spn_cli_run_bin(spn_cli_t* cli) {
  if (cli->num_args < 1) {
    SP_LOG_ERROR("Usage: spn run <bin_name>");
    return;
  }

  const c8* bin_name = cli->args[0];
  sp_str_t name = sp_str_view(bin_name);

  // Find the binary in the project
  spn_bin_t* bin = sp_ht_getp(app.package.bin, name);
  if (!bin) {
    SP_LOG_ERROR("Binary {} not found in project", SP_FMT_STR(name));
    return;
  }

  SP_LOG("Compiling and running {} with TCC", SP_FMT_STR(name));

  // Initialize TCC
  TCCState* tcc = tcc_new();
  if (!tcc) {
    SP_LOG_ERROR("Failed to initialize TCC");
    return;
  }
  ...
  // Get compiler flags from spn print
  // For now, we'll build it manually - in a real implementation we'd use spn_gen_build_entries_for_all

  // Add include directories
  sp_dyn_array_for(bin->include, i) {
    sp_str_t include_path = sp_os_join_path(app.paths.project.dir, bin->include[i]);
    tcc_add_include_path(tcc, include_path.buf);
  }

  // Add dependency includes
  sp_ht_for(app.deps, it) {
    spn_dep_context_t* dep = sp_ht_it_getp(app.deps, it);
    tcc_add_include_path(tcc, dep->paths.include.buf);
  }

  // Add source files
  sp_dyn_array_for(bin->source, i) {
    sp_str_t source_path = sp_os_join_path(app.paths.project.dir, bin->source[i]);
    if (tcc_add_file(tcc, source_path.buf) < 0) {
      SP_LOG_ERROR("Failed to compile {}", SP_FMT_STR(source_path));
      tcc_delete(tcc);
      return;
    }
  }
  ...
  if (tcc_relocate(tcc, TCC_RELOCATE_AUTO) < 0) {
    SP_LOG_ERROR("Failed to relocate code");
    tcc_delete(tcc);
    return;
  }
  ...
  main_fn_t main_fn = (main_fn_t)tcc_get_symbol(tcc, "main");
  if (!main_fn) {
    SP_LOG_ERROR("Failed to find main() function");
    tcc_delete(tcc);
    return;
  }

  // Run the program
  SP_LOG("Executing {}...", SP_FMT_STR(name));

  // Pass remaining arguments to the program
  int prog_argc = cli->num_args;
  char** prog_argv = (char**)cli->args;

  int result = main_fn(prog_argc, prog_argv);

  SP_LOG("Program exited with code {}", SP_FMT_S32(result));
