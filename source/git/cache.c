...
void spn_git_cache_init(spn_git_cache_t* cache, sp_str_t root) {
  *cache = (spn_git_cache_t) {
    .root = root,
    .db.dir = sp_fs_join_path(root, SP_LIT("db")),
    .checkouts.dir = sp_fs_join_path(root, SP_LIT("checkouts")),
  };
...
spn_err_t spn_git_cache_ensure_db(spn_git_cache_t* cache, sp_str_t url, spn_git_db_t** out) {
  *out = SP_NULLPTR;
  ...
  sp_str_t path = sp_fs_join_path(cache->db.dir, key);
...
spn_err_t spn_git_db_ensure_rev(spn_git_db_t* db, sp_str_t rev) {
  ...
  result = sp_ps_run((sp_ps_config_t) {
    .command = SP_LIT("git"),
    .args = {
      SP_LIT("-C"), db->path,
...
spn_err_t spn_git_cache_ensure_checkout(spn_git_cache_t* cache, spn_git_checkout_id_t id, spn_git_checkout_t** out) {
  *out = SP_NULLPTR;
  ...
  sp_str_t path = sp_fs_join_path(cache->checkouts.dir, key);
  ...
  if (!sp_fs_is_dir(path)) {
    ...
    if (!sp_str_empty(id.dir)) {
      sp_str_t subdir_path = sp_fs_join_path(path, id.dir);
  ...
  if (!sp_str_empty(id.dir)) {
    checkout_root = sp_fs_join_path(path, id.dir);
  }
