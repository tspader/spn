...
static void collect(spn_session_t* session, spn_pkg_unit_t* pkg, sp_da(spn_pkg_unit_t*)* visited, sp_da(spn_pkg_unit_t*)* post) {
  sp_da(spn_pkg_unit_t*) deps = spn_session_pkg_deps(session, pkg);
  sp_da_for(deps, it) {
    spn_pkg_unit_t* dep = deps[it];
    if (!dep || dep == pkg) continue;
    if (closure_has_pkg(*visited, dep)) continue;
    sp_da_push(*visited, dep);
    ...
    if (!pkg_is_shared_boundary(dep)) {
      collect(session, dep, visited, post);
    }
...
sp_da(spn_pkg_unit_t*) spn_target_link_closure(sp_mem_t mem, spn_target_unit_t* root) {
  ...
  sp_da(spn_pkg_unit_t*) post = sp_da_new(mem, spn_pkg_unit_t*);
  collect(root->session, root->pkg, &visited, &post);
  ...
  sp_da(spn_pkg_unit_t*) out = sp_da_new(mem, spn_pkg_unit_t*);
