...
spn_err_t add_package(spn_build_graph_t* graph, spn_pkg_unit_t* unit) {
  ...
  // The build script compiles right after the main stamp: build deps have
  // published their headers by then, and user nodes can resolve fns from the
  // module. The flip side is that it can't include headers generated by this
  // package's own nodes, which run after it.
  bool has_build_script = spn_wasm_enabled() && sp_fs_is_file(unit->paths.build);
  if (has_build_script) {
