...
// IDENTITIES //
////////////////
static void dag_hash_invocation(spn_sha256_ctx_t* ctx, const spn_dag_roots_t* roots, const spn_toolchain_unit_t* toolchain, const spn_invocation_t* invocation) {
...
static spn_dag_digest_t dag_compile_identity(spn_dag_build_t* b, const spn_compile_unit_t* unit) {
  sp_assert(!sp_str_empty(unit->base.program));
  spn_sha256_ctx_t ctx = sp_zero;
  spn_sha256_init(&ctx);
  spn_dag_hash_str(&ctx, sp_str_lit("spn.build.compile.v3"));
  dag_hash_invocation(&ctx, &b->roots, unit->target->pkg->build->toolchain, &unit->base);
  spn_dag_hash_masked(&ctx, &b->roots, unit->paths.file);
  return spn_dag_hash_final(&ctx);
}

static spn_err_t dag_link_identity(spn_dag_build_t* b, spn_target_unit_t* target, spn_dag_digest_t* identity) {
  ...
  if (!err.kind) {
    spn_sha256_ctx_t ctx = sp_zero;
    spn_sha256_init(&ctx);
    spn_dag_hash_str(&ctx, sp_str_lit("spn.build.link.v2"));
    dag_hash_invocation(&ctx, &b->roots, target->pkg->build->toolchain, &invocation);
...
static spn_dag_digest_t dag_tree_identity(spn_dag_build_t* b, spn_pkg_unit_t* unit) {
  spn_sha256_ctx_t ctx = sp_zero;
  spn_sha256_init(&ctx);
  ...
  sp_da_for(unit->user_nodes, it) {
    ...
    sp_da_for(node->outputs, ot) {
      if (dag_path_within(node->outputs[ot], unit->paths.include)) {
        spn_dag_hash_masked(&ctx, &b->roots, node->outputs[ot]);
      }
    }
  }
...
static spn_dag_digest_t dag_user_identity(spn_dag_build_t* b, spn_user_node_t* node) {
  spn_sha256_ctx_t ctx = sp_zero;
  spn_sha256_init(&ctx);
  ...
  spn_dag_hash_masked_strs(&ctx, &b->roots, node->inputs);
  spn_dag_hash_masked_strs(&ctx, &b->roots, node->outputs);
  return spn_dag_hash_final(&ctx);
}
...
static s32 dag_compile_exec(spn_dag_t* g, spn_dag_action_t* action, void* user_data) {
...
static spn_err_t dag_compile_discover(spn_dag_t* g, spn_dag_action_t* action, void* user_data, sp_mem_t mem, sp_da(spn_dag_obs_t)* out) {
  spn_compile_unit_t* unit = (spn_compile_unit_t*)user_data;
...
static s32 dag_link_exec(spn_dag_t* g, spn_dag_action_t* action, void* user_data) {
  ...
  return spn_link_target_run(target, dag_artifact_path(g, action->produces[0]), objects);
}

static s32 dag_embed_exec(spn_dag_t* g, spn_dag_action_t* action, void* user_data) {
...
static spn_err_t dag_add_user_nodes(spn_dag_build_t* b, spn_pkg_unit_t* unit, spn_dag_pkg_ids_t* pkg) {
  ...
  if (unit->metaprogram.build.target) {
    spn_dag_target_ids_t* ids = sp_ht_getp(b->ids.targets, unit->metaprogram.build.target);
    if (ids) {
  ...
  sp_da_for(unit->user_nodes, it) {
    ...
    spn_dag_id_t action = spn_dag_add_action(g, (spn_dag_action_config_t) {
      .identity = dag_user_identity(b, node),
...
static spn_err_t dag_add_objects(spn_dag_build_t* b, spn_target_unit_t* target) {
  ...
  sp_da_for(target->objects, it) {
    ...
    ids.action = spn_dag_add_action(g, (spn_dag_action_config_t) {
      .identity = dag_compile_identity(b, unit),
...
spn_err_t spn_dag_build_add_target(spn_dag_build_t* b, spn_target_unit_t* target) {
  ...
  if (!sp_da_empty(target->info->embed)) {
    ...
    ids.embed.action = spn_dag_add_action(g, (spn_dag_action_config_t) {
      .identity = dag_embed_identity(b, target),
  ...
  spn_try(dag_link_identity(b, target, &identity));
  ...
  ids.output = spn_dag_add_file(g, get_target_output_path(b->mem, target));
...
static spn_err_t dag_add_tree(spn_dag_build_t* b, spn_pkg_unit_t* unit, spn_dag_pkg_ids_t* pkg) {
  ...
  spn_dag_id_t action = spn_dag_add_action(g, (spn_dag_action_config_t) {
    .identity = dag_tree_identity(b, unit),
