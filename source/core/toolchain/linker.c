...
static spn_ld_family_t fallback(spn_cc_driver_t driver, spn_ld_dialect_t dialect) {
  switch (driver) {
    ...
    case SPN_CC_DRIVER_MSVC: {
      if (dialect == SPN_LD_DIALECT_GNU) return SPN_LD_FAMILY_GNU;
      else if (dialect == SPN_LD_DIALECT_LINK) return SPN_LD_FAMILY_MSVC;
      else if (dialect == SPN_LD_DIALECT_DARWIN) return SPN_LD_FAMILY_LD64;
      else if (dialect == SPN_LD_DIALECT_WASM) return SPN_LD_FAMILY_LLD;
    }
