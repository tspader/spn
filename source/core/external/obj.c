...
spn_err_t spn_obj_write(spn_obj_builder_t* obj, sp_str_t path) {
  sp_err_t err = SP_OK;
  switch (obj->kind) {
    case SPN_FORMAT_COFF:  err = sp_coff_write_to_file(obj->coff.coff, path); break;
    case SPN_FORMAT_ELF:   err = sp_elf_write_to_file(obj->elf.elf, path); break;
    case SPN_FORMAT_MACHO: err = sp_macho_write_to_file(obj->macho.macho, path); break;
    case SPN_FORMAT_WASM:
