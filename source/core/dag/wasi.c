...
static void wasi_push(spn_dag_wasi_t* w, spn_dag_obs_kind_t kind, sp_str_t host, sp_str_t filter) {
  ...
  sp_da_push(*w->obs, ((spn_dag_obs_t) {
    .kind = kind,
    .path = spn_path_copy(w->obs_mem, spn_path_make(w->roots, host)),
    .filter = sp_str_copy(w->obs_mem, filter)
  }));
}
...
static void wasi_on_readdir(spn_dag_wasi_t* w, u32 fd, u16 error) {
  ...
  sp_str_t host = wasi_host_path(w, s.mem, *guest);
  if (!sp_str_empty(host)) {
    wasi_push_obs(w, SPN_DAG_OBS_ENUMERATION, spn_dag_wasi_canonicalize(s.mem, host));
  }
  sp_mem_end_scratch(s);
}
