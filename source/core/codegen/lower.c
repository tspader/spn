...
static void lower_toolchains(spn_toml_loader_t* ctx, const spn_cg_manifest_t* cg, spn_pkg_info_t* out) {
  ...
  sp_da_for(cg->toolchain, n) {
    ...
    if (toolchain.driver) {
      toolchain.linkers = lower_linkers(ctx, &t->linker, toolchain.driver);
    }
...
static void lower_patches(spn_toml_loader_t* ctx, const spn_cg_manifest_t* cg, spn_pkg_info_t* out) {
  ...
  sp_da_for(cg->patch, it) {
    sp_str_t qualified = lower_canonicalize(ctx, cg->patch[it].key);

    bool duplicate = false;
    sp_da_for(seen, jt) {
      if (sp_str_equal(seen[jt], qualified)) {
        duplicate = true;
        break;
      }
    }
