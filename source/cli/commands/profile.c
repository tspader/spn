...
static sp_cli_result_t invalid(sp_str_t value, const c8* flag, const c8* expected) {
  return spn_cli_usage("invalid value {.red} for {.yellow}; expected {}",
    sp_fmt_str(value),
    sp_fmt_cstr(flag),
...
sp_cli_result_t spn_cli_parse_profile(spn_profile_override_t* profile) {
  ...
  if (!sp_str_empty(cmd->target) && spn_triple_parse(cmd->target, &target)) {
    return invalid(cmd->target, "--target", "an <arch>-<os>-<abi> triple like x86_64-linux-gnu");
  }
  ...
  if (sp_str_equal_cstr(cmd->sanitize, "none")) {
    ...
    sp_da_for(names, it) {
      ...
      if (!sanitizer) {
        ...
        return invalid(cmd->sanitize, "--sanitize", "a comma-separated list of address, thread, undefined, memory, leak, or none");
      }
      sanitizers |= sanitizer;
    }
    sp_mem_end_scratch(scratch);
    if (spn_sanitizer_set_has_conflict(sanitizers)) {
      return invalid(cmd->sanitize, "--sanitize", "a compatible set (thread and memory don't combine with each other, address, or leak)");
    }
  }
