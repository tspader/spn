...
s32 spn_autoconf_run(spn_autoconf_t* autoconf) {
  ...
  sp_ps_config_t config = {
    .command = sp_fs_join_path(spn_allocator, unit->paths.source, sp_str_lit("configure")),
    .args = {
  ...
  if (!spn_triple_match(target, host) || !spn_triple_match(host, target)) {
    sp_ps_config_add_arg(spn_allocator, &config, sp_format("--build={}", SP_FMT_STR(spn_triple_to_autoconf(host))));
    sp_ps_config_add_arg(spn_allocator, &config, sp_format("--host={}", SP_FMT_STR(spn_triple_to_autoconf(target))));
  }

  sp_da_for(autoconf->flags, it) {
    sp_ps_config_add_arg(spn_allocator, &config, autoconf->flags[it]);
  }
...
void spn_autoconf_add_flag(spn_autoconf_t* autoconf, const c8* flag) {
  sp_da_push(autoconf->flags, sp_str_from_cstr(spn_allocator, flag));
}
