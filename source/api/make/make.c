...
void spn_make_add_target(spn_make_t* make, const c8* target) {
  make->target = sp_str_from_cstr(spn_mem_todo, target);
}
...
s32 spn_make_run(spn_make_t* make) {
  ...
  sp_ps_config_add_arg(spn_mem_todo, &ps, sp_str_lit("--quiet"));
  sp_ps_config_add_arg(spn_mem_todo, &ps, sp_str_lit("--directory"));
  sp_ps_config_add_arg(spn_mem_todo, &ps, unit->paths.work);
  if (!sp_str_empty(make->target)) {
    sp_ps_config_add_arg(spn_mem_todo, &ps, make->target);
  }
