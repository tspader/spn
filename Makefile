...
IS_SPN_PREINSTALLED := $(shell which $(PREINSTALLED_SPN) 2>NUL)
...
SPN_DIR_BUILD:= build
  SPN_DIR_BUILD_BOOTSTRAP:= $(SPN_DIR_BUILD)/bootstrap
		...
		SPN_BOOTSTRAP_BIN := $(SPN_DIR_BUILD_BOOTSTRAP)/bin
			SPN_BOOTSTRAP_BIN_SDL := $(SPN_BOOTSTRAP_BIN)/$(SDL)
			SPN_BOOTSTRAP_BIN_LUAJIT := $(SPN_BOOTSTRAP_BIN)/$(LUAJIT)
...
BOOTSTRAP_INCLUDE := -I$(SPN_BOOTSTRAP_SDL)/include -I$(SPN_BOOTSTRAP_SP) -I$(SPN_BOOTSTRAP_TOML) -I$(SPN_BOOTSTRAP_ARGPARSE) -I build/bootstrap/LuaJIT/src
BOOTSTRAP_LIBS := $(SPN_BOOTSTRAP_BIN_SDL) build/bootstrap/LuaJIT/src/lua51.dll
...
$(SPN_BOOTSTRAP_WORK): $(SPN_DIR_BUILD_BOOTSTRAP)
	mkdir -p $(SPN_DIR_BUILD_BOOTSTRAP)/work
...
$(SPN_BOOTSTRAP_BIN_LUAJIT): | $(SPN_BOOTSTRAP_LUAJIT)
	@make -C $(SPN_BOOTSTRAP_LUAJIT)
	-@cp $(SPN_BOOTSTRAP_LUAJIT)/src/*.dll $(SPN_BOOTSTRAP_BIN)
	-@cp $(SPN_BOOTSTRAP_LUAJIT)/src/*.so $(SPN_BOOTSTRAP_BIN)
	-@cp $(SPN_BOOTSTRAP_LUAJIT)/src/*.lib $(SPN_BOOTSTRAP_BIN)
	-@cp $(SPN_BOOTSTRAP_LUAJIT)/src/*.a $(SPN_BOOTSTRAP_BIN)

$(SPN_BOOTSTRAP_BIN_SDL): | $(SPN_BOOTSTRAP_SDL) $(SPN_BOOTSTRAP_WORK)
	...
	-@cp $(SPN_BOOTSTRAP_WORK)/*.dll $(SPN_BOOTSTRAP_BIN)
	-@cp $(SPN_BOOTSTRAP_WORK)/*.so $(SPN_BOOTSTRAP_BIN)
	-@cp $(SPN_BOOTSTRAP_WORK)/*.lib $(SPN_BOOTSTRAP_BIN)
	-@cp $(SPN_BOOTSTRAP_WORK)/*.a $(SPN_BOOTSTRAP_BIN)

SPN_BOOTSTRAP_DEPS := $(SPN_BOOTSTRAP_SP) $(SPN_BOOTSTRAP_ARGPARSE) $(SPN_BOOTSTRAP_TOML) $(SPN_DIR_BUILD_OUTPUT)

bootstrap: $(SPN_BOOTSTRAP_BIN_SDL) $(SPN_BOOTSTRAP_BIN_LUAJIT) | $(SPN_BOOTSTRAP_DEPS)
	@echo ">> bootstrapping spn"
	exit 0
	$(CC) $(CC_FLAGS) $(BOOTSTRAP_FLAGS) ./source/main.c
	@echo
