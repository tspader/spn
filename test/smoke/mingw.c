...
sp_test(mingw, gnu_script) {
  return run_test(t, (test_t) {
    ...
    .copy = { "main.ld" },
    .when = { .toolchain = "mingw-gnu", .programs = { "x86_64-w64-mingw32-gcc", "x86_64-w64-mingw32-ar" } },
    .actions = {
...
sp_test(mingw, clang_gnu) {
  return run_test(t, (test_t) {
    ...
    .when = { .toolchain = "clang-mingw", .programs = { "clang", "x86_64-w64-mingw32-ld", "x86_64-w64-mingw32-ar" } },
    .actions = {
      { .kind = ACTION_RUN_CLI, .cli = { .cmd = "build", .args = { "--target", "x86_64-windows-gnu" } } },
      { .kind = ACTION_VERIFY_EVENT, .verify_event = { .event = SPN_EVENT_LINK_PASSED } },
    },
