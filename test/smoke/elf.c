...
sp_test(elf, gnu_script) {
  return run_test(t, (test_t) {
    ...
    .copy = { "main.ld" },
    .when = { .toolchain = "gnu", .host = SPN_OS_LINUX, .programs = { "gcc", "ar" } },
    .actions = {
...
sp_test(elf, lld_script) {
  return run_test(t, (test_t) {
    ...
    .copy = { "main.ld" },
    .when = { .toolchain = "gcc-lld", .host = SPN_OS_LINUX, .programs = { "gcc", "ar", "ld.lld" } },
    .actions = {
      { .kind = ACTION_RUN_CLI, .cli = { .cmd = "build", .args = { "--target", SPN_TEST_ARCH "-freestanding" } } },
      { .kind = ACTION_VERIFY_ELF_ENTRY, .verify_elf_entry = { target_exe("main", SPN_TEST_ARCH "-freestanding-none"), 0x400000 } },
    },
...
sp_test(elf, clang_gnu) {
  return run_test(t, (test_t) {
    ...
    .when = { .toolchain = "clang-gnu", .host = SPN_OS_LINUX, .programs = { "clang", "ld", "ar" } },
    .actions = {
...
sp_test(elf, clang_lld) {
  return run_test(t, (test_t) {
    ...
    .when = { .toolchain = "clang-lld", .host = SPN_OS_LINUX, .programs = { "clang", "ld.lld", "llvm-ar" } },
    .actions = {
      { .kind = ACTION_RUN_CLI, .cli = { .cmd = "build" } },
      { .kind = ACTION_VERIFY_EVENT, .verify_event = { .event = SPN_EVENT_LINK_PASSED } },
    },
...
sp_test(elf, cross_gnu_script) {
  return run_test(t, (test_t) {
    ...
    .copy = { "main.ld" },
    .when = { .toolchain = "aarch64-gnu", .programs = { "aarch64-linux-gnu-gcc", "aarch64-linux-gnu-ar" } },
    .actions = {
