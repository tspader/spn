...
static sp_str_t build_release_json(sp_str_t name, sp_str_t version, sp_str_t repo_url, sp_str_t commit) {
  ...
  sp_str_builder_append(&b, sp_str_replace_c8(repo_url, '\\', '/'));
...
void setup_fixture_index_from_remote(s32* result, tmpfs_t* fs, sp_str_t index, sp_str_t project) {
  ...
  sp_str_t core_dir = sp_fs_join_path(index, sp_str_lit("core"));
  sp_fs_create_dir(core_dir);

  sp_da(sp_fs_entry_t) entries = sp_fs_collect(remote);
  sp_da_for(entries, it) {
    ...
    sp_da(sp_fs_entry_t) versions = sp_fs_collect(entry->file_path);
    ASSERT_FALSE(sp_da_empty(versions));
    sp_dyn_array_sort(versions, sort_dirs_by_name);

    sp_str_t repo = tmpfs_get(fs, sp_fs_join_path(sp_str_lit("remote"), entry->file_name));
    sp_str_t jsonl_path = sp_fs_join_path(core_dir, sp_format("{}.jsonl", SP_FMT_STR(entry->file_name)));
    ...
    sp_da_for(versions, v) {
      ...
      git_repo_commit_from_dir(dir->file_path, repo, dir->file_name);
      sp_str_t commit = git_repo_head(repo);

      sp_str_t line = build_release_json(entry->file_name, dir->file_name, repo, commit);
...
void setup_fixture_config(tmpfs_t* fs, sp_str_t config_dir, sp_str_t index_dir, sp_str_t spn_dir) {
  sp_str_t spn_config_dir = sp_fs_join_path(config_dir, sp_str_lit("spn"));
  sp_fs_create_dir(spn_config_dir);

  sp_str_t config_path = sp_fs_join_path(spn_config_dir, sp_str_lit("spn.toml"));
  sp_str_t content = sp_format(
    "spn = \"{}\"\n"
    "\n"
    "[[index]]\n"
    ...
    "url = \"{}\"\n"
    "protocol = \"filesystem\"\n",
    SP_FMT_STR(sp_str_replace_c8(spn_dir, '\\', '/')),
    SP_FMT_STR(sp_str_replace_c8(index_dir, '\\', '/'))
  );
