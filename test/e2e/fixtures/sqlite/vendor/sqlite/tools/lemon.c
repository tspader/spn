...
void lemon_reset(void);   /* zero persistent global state so lemon_main is re-entrant */
int lemon_main(int argc, char **argv){
  ...
  /* Re-entrancy: this program was written to run once per process. When called
  ** repeatedly in one wasm instance (parse.y then fts5parse.y), the persistent
  ** intern/symbol/state/config tables and the option flags below would carry
  ** over and corrupt the second grammar. Reset them on every entry. All our
  ** invocations pass identical flags, but reset defensively regardless. */
  version = rpflag = basisflag = compress = quiet = statistics = 0;
  mhflag = nolinenosflag = noResort = sqlFlag = printPP = 0;
  lemon_reset();
  ...
  */
  ...
  lemon_free_all();
  /* spn: was exit(exitcode); return instead so lemon_main can be called more
  ** than once in a single wasm instance without terminating it. */
  return (exitcode);
}
...
/* Added for spn: reset lemon's persistent file-scope state between runs in a
** single process/wasm instance. Zeroing the four table pointers makes the
** Strsafe/Symbol/State/Configtable *_init guards ("if(xNa) return") re-allocate
** fresh tables. The old allocations leak, which is fine for a one-shot build. */
void lemon_reset(void) {
  x1a = 0;   /* string intern table  (Strsafe) */
  x2a = 0;   /* symbol table         (Symbol)  */
  x3a = 0;   /* state table          (State)   */
  x4a = 0;   /* config table         (Configtable) */
  nDefine = 0;
  nDefineUsed = 0;
}
