...
UTEST_F(e2e, bloodgun) {
  ...
  run_actions(utest_result, &uf->fixture, (action_t[]) {
    ...
    { .kind = ACTION_RUN_CLI, .cli = { "build" } },
    { .kind = ACTION_VERIFY_EXISTS, .verify_exists.file = sp_str_lit("build/debug/store/bin/bitgun-survivors") },
    ...
    { .kind = ACTION_VERIFY_EXISTS, .verify_exists.file = sp_str_lit("build/profiling/store/bin/bitgun-survivors") },
