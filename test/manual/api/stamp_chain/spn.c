...
spn_err_t step1_fn(spn_node_ctx_t* ctx) {
  order[order_idx++] = 1;
  spn_log(ctx->build, "step1");
  return SPN_OK;
}

spn_err_t step2_fn(spn_node_ctx_t* ctx) {
  order[order_idx++] = 2;
  spn_log(ctx->build, "step2");
  return SPN_OK;
}

spn_err_t step3_fn(spn_node_ctx_t* ctx) {
  order[order_idx++] = 3;
  spn_log(ctx->build, "step3");
  return SPN_OK;
}

spn_err_t final_fn(spn_node_ctx_t* ctx) {
  order[order_idx++] = 4;
  spn_log(ctx->build, "final: writing result.h");
  spn_write_file(ctx->build, "result.h",
    "#ifndef RESULT_H\n"
    "#define RESULT_H\n"
