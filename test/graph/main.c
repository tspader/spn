...
spn_build_graph_t* create_split_join_graph() {
  ...
  spn_bg_id_t f = spn_bg_add_file(g, sp_str_lit("f"));

  spn_bg_id_t split = spn_bg_add_command(g, SPN_BUILD_CMD_FN);
  spn_bg_id_t join_1 = spn_bg_add_command(g, SPN_BUILD_CMD_FN);
  spn_bg_id_t join_2 = spn_bg_add_command(g, SPN_BUILD_CMD_FN);

  spn_bg_tag_command_c(g, split, "split");
  spn_bg_tag_command_c(g, join_1, "join");
  spn_bg_tag_command_c(g, join_2, "join");

  spn_build_command_add_input(g, split, a);
  spn_build_command_add_output(g, split, b);
  spn_build_command_add_output(g, split, c);

  spn_build_command_add_input(g, join_1, b);
  spn_build_command_add_input(g, join_1, c);
  spn_build_file_set_command(g, d, join_1);

  spn_build_command_add_input(g, join_2, d);
  spn_build_command_add_input(g, join_2, e);
  spn_build_file_set_command(g, f, join_2);
