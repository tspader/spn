...
spn_bg_id_t spn_bg_add_file(spn_build_graph_t* graph, sp_str_t path) {
  spn_build_file_t file = SP_ZERO_STRUCT(spn_build_file_t);
  file.id.index = sp_da_size(graph->files);
  file.id.occupied = true;
  file.path = path;
...
spn_bg_id_t spn_bg_add_command(spn_build_graph_t* graph, spn_build_cmd_kind_t kind) {
  spn_build_cmd_t cmd = SP_ZERO_STRUCT(spn_build_cmd_t);
  cmd.id.index = sp_da_size(graph->commands);
  cmd.id.occupied = true;
  cmd.kind = kind;
