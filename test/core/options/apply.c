...
typedef struct {
  const c8* name;
  apply_clause_t when [2];
} apply_gated_target_t;

typedef struct {
  const c8* name;
  spn_when_facts_t facts;
  apply_gated_target_t targets [4];
  struct { const c8* targets [4]; } expect;
} apply_gate_test_t;

static const apply_gate_test_t gate_tests [] = {
  {
    .name = "ungated_kept",
    .facts = { .os = SPN_OS_LINUX },
    .targets = { { "A" } },
  ...
  {
    .name = "failing_dropped",
    .facts = { .os = SPN_OS_LINUX },
    .targets = { { "A", { { "os", "windows" } } } },
  },
  {
    .name = "survivors_keep_order",
    .facts = { .os = SPN_OS_LINUX },
    .targets = {
      { "A", { { "os", "linux" } } },
      { "B", { { "os", "windows" } } },
...
sp_test_each(options_apply, gated_targets, apply_gate_test_t, gate_tests) {
  sp_mem_t mem = sp_test_arena(t);
  spn_pkg_info_t info = sp_zero;
  spn_target_map_t* maps [] = { &info.libs, &info.exes, &info.scripts, &info.tests, &info.examples };

  u32 declared = 0;
  sp_carr_detect_len(it->targets, declared, it->targets[declared].name);
  sp_carr_for(maps, mt) {
    sp_for(tt, declared) {
      spn_target_info_t target = {
        .name = sp_cstr_as_str(it->targets[tt].name),
        .when = make_apply_when(mem, it->targets[tt].when, SP_CARR_LEN(it->targets[tt].when)),
      };
      sp_str_om_insert(*maps[mt], target.name, target);
    }
  }
  ...
  u32 expected = 0;
  sp_carr_detect_len(it->expect.targets, expected, it->expect.targets[expected]);
  sp_carr_for(maps, mt) {
    sp_must_eq(t, sp_str_om_size(*maps[mt]), expected);
    sp_for(et, expected) {
      sp_expect_str_eq_c(t, sp_str_om_at(*maps[mt], et)->name, it->expect.targets[et]);
    }
    sp_for(tt, declared) {
      sp_str_t name = sp_cstr_as_str(it->targets[tt].name);
      bool kept = false;
      sp_for(et, expected) {
        kept |= sp_str_equal_cstr(name, it->expect.targets[et]);
      }
      sp_expect_eq(t, sp_str_om_has(*maps[mt], name), kept);
    }
  }
  return SP_OK;
}
