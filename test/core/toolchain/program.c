...
static const test_t tests [] = {
  ...
  { "local_absolute_ignores_base",       SPN_TOOLCHAIN_SOURCE_LOCAL,        SPN_PATH_ROOT_PROJECT, "/A/B",   .expect = { .arg = { .path = "/A/B" } } },
  ...
  { "local_relative_without_base",       SPN_TOOLCHAIN_SOURCE_LOCAL,        SPN_PATH_ROOT_NONE,    "A/B",    .expect = { .check = SPN_PROGRAM_UNROOTED } },
  { "local_malformed",                   SPN_TOOLCHAIN_SOURCE_LOCAL,        SPN_PATH_ROOT_PROJECT, "./A",    .expect = { .check = SPN_PROGRAM_MALFORMED } },
  { "local_dot_name",                    SPN_TOOLCHAIN_SOURCE_LOCAL,        SPN_PATH_ROOT_PROJECT, ".",      .expect = { .check = SPN_PROGRAM_MALFORMED } },
  ...
  { "distribution_absolute",             SPN_TOOLCHAIN_SOURCE_DISTRIBUTION, SPN_PATH_ROOT_NONE,    "/A",     .expect = { .check = SPN_PROGRAM_UNROOTED } },
  { "distribution_malformed",            SPN_TOOLCHAIN_SOURCE_DISTRIBUTION, SPN_PATH_ROOT_NONE,    "A/../B", .expect = { .check = SPN_PROGRAM_MALFORMED } },
};

sp_test_each(program, classify, test_t, tests) {
  spn_arg_t arg = sp_zero;
  spn_program_check_t check = spn_toolchain_program(it->source, it->base, sp_cstr_as_str(it->program), &arg);
