...
case "$TARGET" in
  aarch64-macos) ASSET="spn-aarch64-macos.tar.gz" SHA="aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa" EXE="spn" EXTRACT=extract_tar ;;
  ...
  x86_64-windows) ASSET="spn-x86_64-windows.zip" SHA="cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc" EXE="spn.exe" EXTRACT=extract_zip ;;
  *) fail "spn ${VERSION} has no build for ${TARGET}; it ships for: aarch64-macos x86_64-linux x86_64-windows" ;;
esac
