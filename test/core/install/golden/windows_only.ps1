...
$Tmp = New-Item -ItemType Directory -Path (Join-Path ([IO.Path]::GetTempPath()) "spn-install-$([Guid]::NewGuid())")
$Archive = Join-Path $Tmp $Target.Asset
...
try {
  Move-Item -Force (Join-Path $Tmp $Target.Exe) $Exe
} catch {
