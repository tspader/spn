...
static void compare_ref_ok(s32* utest_result, const jtd_root_t* root, const void* expect) {
...
static void compare_ref_chain(s32* utest_result, const jtd_root_t* root, const void* expect) {
  ...
  jtd_schema_t* deep = jtd_resolve_deep(root, root->root, &diag);
...
static void compare_ref_cycle(s32* utest_result, const jtd_root_t* root, const void* expect) {
  jtd_diagnostic_t diag = SP_ZERO_INITIALIZE();
  jtd_schema_t* deep = jtd_resolve_deep(root, root->root, &diag);
  EXPECT_TRUE(deep == SP_NULLPTR);
...
static void compare_ref_recursive_properties(s32* utest_result, const jtd_root_t* root, const void* expect) {
  jtd_schema_t* deep = jtd_resolve_deep(root, root->root, SP_NULLPTR);
  EXPECT_TRUE(deep != SP_NULLPTR);
  if (deep) {
    ...
    if (sp_da_size(deep->as.properties.optional) == 1) {
      ...
      jtd_schema_t* resolved = jtd_resolve_deep(root, next->schema, SP_NULLPTR);
