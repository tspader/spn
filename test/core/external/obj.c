...
static const machine_t machine_tests [] = {
  { "elf_x86_64",    SPN_FORMAT_ELF,   SPN_ARCH_X64,   EM_X86_64 },
  { "elf_aarch64",   SPN_FORMAT_ELF,   SPN_ARCH_ARM64, EM_AARCH64 },
  { "coff_x86_64",   SPN_FORMAT_COFF,  SPN_ARCH_X64,   SP_COFF_MACHINE_AMD64 },
  { "coff_aarch64",  SPN_FORMAT_COFF,  SPN_ARCH_ARM64, SP_COFF_MACHINE_ARM64 },
  { "macho_x86_64",  SPN_FORMAT_MACHO, SPN_ARCH_X64,   SP_MACHO_CPU_X86_64 },
  { "macho_aarch64", SPN_FORMAT_MACHO, SPN_ARCH_ARM64, SP_MACHO_CPU_ARM64 },
};

static sp_err_t write_obj(spn_obj_builder_t* obj, sp_io_writer_t* out) {
  switch (obj->kind) {
    case SPN_FORMAT_COFF:  return sp_coff_write(obj->coff.coff, out);
    case SPN_FORMAT_ELF:   return sp_elf_write(obj->elf.elf, out);
    case SPN_FORMAT_MACHO: return sp_macho_write(obj->macho.macho, out);
...
static u32 read_machine(spn_format_t kind, sp_str_t bytes) {
  u16 half = 0;
  u32 word = 0;
  switch (kind) {
