...
static const enum_test_t enum_tests [] = {
  ...
  {
    .name = "member_type_change_reruns",
    .obs = { { "A", "*.h" } },
    .runs = {
      { .files = { "A/X.h" }, .expect_runs = 1 },
  ...
  {
    .name = "path_is_file_hashes_empty",
    .obs = { { "A", "*.h" } },
    .runs = {
  ...
  {
    .name = "manifest_reload_roundtrip",
    .obs = { { "A", "*.h" } },
    .runs = {
      { .files = { "A/X.h" }, .expect_runs = 1 },
...
static void enum_prepare(enum_env_t* env, const enum_run_t* run) {
  ...
  sp_carr_for(run->files, it) {
    ...
    dag_test_env_create(&env->dag, sp_str_view(run->files[it]), sp_str_lit("S"));
  }
