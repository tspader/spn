...
s32 phase1_fn(spn_node_ctx_t* ctx) {
  spn_log(spn_node_ctx_get_build(ctx), "phase1: generating intermediate.h...");
  spn_write_file(spn_node_ctx_get_build(ctx), "intermediate.h",
...
s32 phase2_fn(spn_node_ctx_t* ctx) {
  spn_log(spn_node_ctx_get_build(ctx), "phase2: generating final.h...");
