/* spn: enable wasi-libc emulated signal/getpid decls (funcs are GC'd at link). */
#ifdef __wasi__
#define _WASI_EMULATED_SIGNAL 1
#define _WASI_EMULATED_GETPID 1
#endif
/* spn: when set by the build driver, this interp's stdin/stdout channels are
** opened on these files instead of the inherited fds (see JimAioInit). */
const char* spn_jimsh_stdin_path = 0;
const char* spn_jimsh_stdout_path = 0;
...
#endif
#ifdef __wasi__
/* WASI has no fork/exec/pipe/waitpid; drop those features. The exec code
** still compiles (as the system()-based variant) but is GC'd at link time
** because we don't register the exec command below. WASI also lacks
** umask/mkstemp (no temp-file command) and a working setjmp/longjmp. */
#undef HAVE_FORK
#undef HAVE_WAITPID
#undef HAVE_PIPE
#undef HAVE_UMASK
...
#endif
#ifdef __wasi__
/* On wasi-libc the CLOCK_* macros expand to pointer-typed clockid values,
** but Jim_GetTimeUsec() takes an unsigned int. Since HAVE_CLOCK_GETTIME is
** off (it falls back to gettimeofday) the value is ignored; force plain ints
** so the call sites type-check. */
...
int Jim_aioInit(Jim_Interp *interp)
{
    ...
    /* spn: the build driver redirects this interp's stdin/stdout to work-dir
    ** files by setting these globals, instead of freopen()ing the shared fds
    ** (freopen double-closes under WAMR's WASI fd table). When set, we open the
    ** file for this channel and let the interp close it on teardown (no
    ** AIO_KEEPOPEN); otherwise the real stdio fd is wrapped as usual. */
    if (spn_jimsh_stdin_path) {
        int fd = open(spn_jimsh_stdin_path, O_RDONLY);
        JimMakeChannel(interp, fd >= 0 ? fd : fileno(stdin), NULL, "stdin", 0, fd >= 0 ? 0 : AIO_KEEPOPEN);
    }
    else {
        JimMakeChannel(interp, fileno(stdin), NULL, "stdin", 0, AIO_KEEPOPEN);
    }
    if (spn_jimsh_stdout_path) {
        int fd = open(spn_jimsh_stdout_path, O_WRONLY | O_CREAT | O_TRUNC, 0644);
        JimMakeChannel(interp, fd >= 0 ? fd : fileno(stdout), NULL, "stdout", 0, fd >= 0 ? 0 : AIO_KEEPOPEN);
    }
    else {
...
#ifdef __wasi__
    /* WASI has no working realpath(); do a lexical normalization instead.
    ** Absolute paths are returned as-is; relative paths are anchored at cwd.
    ** Sufficient for the sqlite codegen scripts, which pass absolute paths. */
    if (path[0] == '/') {
        snprintf(newname, MAXPATHLEN, "%s", path);
    }
    else {
        char cwd[MAXPATHLEN];
        if (!getcwd(cwd, sizeof(cwd))) { cwd[0] = '/'; cwd[1] = 0; }
        snprintf(newname, MAXPATHLEN, "%s/%s", cwd, path);
    }
    JimFixPath(newname);
...
#include <setjmp.h>
#else
/* WASI's setjmp/longjmp needs the wasm exception-handling proposal and a
** runtime helper that this libc doesn't provide. jimsh only uses it to
** escape a failing qsort() comparator in [lsort]; emulate that with a
** dummy buffer plus an error flag (see struct lsort_info / ListSort). */
...
struct lsort_info {
    jmp_buf jmpbuf;
#ifdef __wasi__
    int longjmp_rc;             /* nonzero once a comparator has failed */
#endif
    Jim_Obj *command;
    Jim_Interp *interp;
    enum {
...
#ifdef __wasi__
/* Emulated longjmp for the comparators below: record the failure and bail
** out of this comparison returning "equal". ListSort checks longjmp_rc
** after qsort() completes. (val==0 maps to 1, matching longjmp semantics.) */
#define longjmp(buf, val) do { sort_info->longjmp_rc = (val) ? (val) : 1; return 0; } while (0)
#endif
...
int Jim_MakeTempFile(Jim_Interp *interp, const char *filename_template, int unlink_file)
{
    ...
    fd = -1;                    /* WASI provides no temp-file primitive */
#else
