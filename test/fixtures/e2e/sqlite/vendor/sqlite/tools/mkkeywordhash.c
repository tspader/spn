...
/* spn: emit to a file given as argv[1] rather than stdout, so the build driver
** never has to freopen the shared stdout fd (which double-closes under WAMR's
** WASI fd table). All output in this file goes through printf. */
static FILE* g_mkkh_out;
#define printf(...) fprintf(g_mkkh_out, __VA_ARGS__)
...
int mkkeywordhash_main(int argc, char **argv){
  ...
  int nChar;
  int totalLen = 0;
  g_mkkh_out = (argc > 1) ? fopen(argv[1], "w") : stdout;
  if (!g_mkkh_out) return 1;
  ...
  ** number of collisions */
  ...
  if (g_mkkh_out != stdout) fclose(g_mkkh_out);
  return 0;
}
