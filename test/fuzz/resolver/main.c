...
static fz_result_t fz_execute(sp_mem_t mem, fz_universe_t* u) {
  ...
  *root = sp_zero_s(spn_pkg_info_t);
  ...
  spn_resolver_init(&resolver, mem, intern, &cache, &registry, events, SPN_LIB_KIND_NONE, config, 0);
...
static fz_err_t fz_run_iteration(sp_fuzz_prng_t base, u64 iter) {
  ...
  fz_universe_t universe = fz_gen_universe(mem, &prng);
  fz_result_t result = fz_execute(mem, &universe);
  ...
  if (universe.planted && !fz_oracle_sat(&universe)) {
    ...
    err = fz_check_solve(&universe, &result.query);
  }
  else if (sp_da_empty(result.events)) {
    err = FZ_ERR_EVENT_MISSING;
  }
  else if (!fz_pushed_event(&result, SPN_EVENT_ERR_RESOLUTION_TOO_COMPLEX)) {
    if (universe.planted) {
      err = FZ_ERR_PLANTED_REJECTED;
    }
    else if (fz_oracle_sat(&universe)) {
      err = FZ_ERR_INCOMPLETE;
    }
  }

  if (err) {
    fz_dump(&universe, iter);
  }
...
s32 main(void) {
  ...
  sp_da_push(names, sp_str_lit("fuzz/resolver/pure_scope"));
