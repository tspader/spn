...
// only the output invariants and the metamorphic checks apply.
...
#define FZ_LATTICE_SIZE (3 * 4 * 3)

static u64 fz_lattice(spn_semver_t* out) {
  u64 count = 0;
  for (u32 major = 1; major <= 3; major++) {
...
fz_profile_t fz_gen_profile(sp_fuzz_prng_t* prng) {
  ...
  profile.pkg_count = 2 + sp_fuzz_below(prng, FZ_MAX_PKGS - 1);
...
fz_universe_t fz_gen_universe(sp_mem_t mem, sp_fuzz_prng_t* prng, fz_profile_t profile) {
  ...
  for (u64 it = 0; it < count; it++) {
    ...
    pkg.shared = profile.features && sp_fuzz_chance(prng, (u32)profile.shared_pct, 16);
    sp_da_init(mem, pkg.releases);

    u64 releases = 1 + sp_fuzz_below(prng, profile.release_count);
  ...
  sp_da_for(u.pkgs, it) {
    ...
    sp_da_for(pkg->releases, rt) {
      ...
      if (!sp_da_empty(release->deps) && sp_fuzz_chance(prng, 1, 8)) {
        ...
        spn_semver_t target = u.pkgs[dup].releases[assigned[dup]].version;
        sp_da_push(release->deps, on_plan ?
...
// # MUTATORS
//
// These functions take a universe and apply a mutation which changes the
// universe, but in a way that doesn't change the resolve assignment. This
// exists to stress our order independence; we never want a universe to
// resolve when presented in one order but not another
...
// If a universe is satisfiable, then the universe which consists of only
// the packages in the solution should also be satisfiable
...
// Adding a release can never make a satisfiable universe unsatisfiable
fz_universe_t fz_extend_universe(sp_mem_t mem, sp_fuzz_prng_t* prng, fz_universe_t* u) {
  fz_universe_t copy = fz_copy_universe(mem, u);
  u32 target = (u32)sp_fuzz_below(prng, sp_da_size(copy.pkgs));
