...
typedef enum {
  ...
  SP_TEMPLATE_ERR_UNTERMINATED_IF,
  SP_TEMPLATE_ERR_UNEXPECTED_END,
  SP_TEMPLATE_ERR_UNEXPECTED_ELSE,
...
#if defined(SP_TEMPLATE_IMPLEMENTATION) && !defined(SP_TEMPLATE_H_IMPL)
#define SP_TEMPLATE_H_IMPL
...
typedef enum {
  ...
  SP_TEMPLATE_DIR_FOR,
  SP_TEMPLATE_DIR_IF,
  SP_TEMPLATE_DIR_ELSE,
  SP_TEMPLATE_DIR_END,
} sp_template_dir_kind_t;

typedef struct {
  sp_template_dir_kind_t kind;
  sp_str_t name; // bound for SUBST, FOR, and IF
  bool negate;   // bound for IF
} sp_template_dir_t;
...
static sp_template_err_t sp_template_classify(sp_str_t content, sp_template_dir_t* out) {
  ...
  if (sp_str_equal_cstr(keyword, "if")) {
    sp_str_t operand = rest;
    bool negate = operand.len && operand.data[0] == '!';
    if (negate) operand = sp_str_trim(sp_str_sub(operand, 1, (s32)operand.len - 1));
    if (!operand.len || operand.data[0] != '.') return SP_TEMPLATE_ERR_BAD_DIRECTIVE;
    sp_str_t name = sp_str_sub(operand, 1, (s32)operand.len - 1);
    if (!sp_template_name_valid(name)) return SP_TEMPLATE_ERR_BAD_DIRECTIVE;
    out->kind = SP_TEMPLATE_DIR_IF;
    out->name = name;
    out->negate = negate;
    return SP_TEMPLATE_OK;
  }

  if (sp_str_equal_cstr(keyword, "else")) {
    if (rest.len) return SP_TEMPLATE_ERR_BAD_DIRECTIVE;
    out->kind = SP_TEMPLATE_DIR_ELSE;
    return SP_TEMPLATE_OK;
  }
...
static sp_str_t sp_template_lstrip_block(sp_str_t run, bool line_start) {
  u32 tail = run.len;
  while (tail > 0 && (run.data[tail - 1] == ' ' || run.data[tail - 1] == '\t')) tail--;
  if (tail == 0) {
    // A pure-whitespace run is line-leading indentation only if the run itself
    // began at a line start (the previous block directive consumed the newline).
    return line_start ? sp_str_sub(run, 0, 0) : run;
  }
...
static sp_template_err_t sp_template_render_block(
  ...
  *hit_else = false;
  ...
  while (p->i < p->str.len) {
    ...
    bool block = dir.kind == SP_TEMPLATE_DIR_FOR || dir.kind == SP_TEMPLATE_DIR_IF || dir.kind == SP_TEMPLATE_DIR_ELSE || dir.kind == SP_TEMPLATE_DIR_END;

    if (io && literal_end > literal) {
      sp_str_t run = sp_str_sub(p->str, (s32)literal, (s32)(literal_end - literal));
      if (block) run = sp_template_lstrip_block(run, literal == 0 || p->str.data[literal - 1] == '\n');
    ...
    switch (dir.kind) {
      ...
      case SP_TEMPLATE_DIR_INCLUDE: {
        if (io) {
          ...
          bool leaf_end = false;
          bool leaf_else = false;
          err = sp_template_render_block(&leaf, scope, io, reg, depth + 1, &leaf_end, &leaf_else);
          if (err) return err;
          if (leaf_else) return SP_TEMPLATE_ERR_UNEXPECTED_ELSE;
      ...
      case SP_TEMPLATE_DIR_FOR: {
        ...
        if (!io) {
          ...
          bool body_end = false;
          bool body_else = false;
          err = sp_template_render_block(p, SP_NULLPTR, SP_NULLPTR, reg, depth, &body_end, &body_else);
          if (err) return err;
          if (body_else) return SP_TEMPLATE_ERR_UNEXPECTED_ELSE;
        ...
        if (sp_da_empty(value->as.list)) {
          ...
          sp_da_for(value->as.list, child) {
            ...
            bool body_end = false;
            bool body_else = false;
            err = sp_template_render_block(p, value->as.list[child], io, reg, depth, &body_end, &body_else);
            if (err) return err;
            if (body_else) return SP_TEMPLATE_ERR_UNEXPECTED_ELSE;
            if (!body_end) return SP_TEMPLATE_ERR_UNTERMINATED_FOR;
          }
      ...
      case SP_TEMPLATE_DIR_IF: {
        // The untaken branch renders in skip mode -- the same no-io walk an empty
        // for uses -- so nesting and end matching behave identically.
        bool truthy = false;
        if (io) {
          sp_template_value_t* value = sp_template_scope_resolve(scope, dir.name);
          if (value) {
            switch (value->kind) {
              case SP_TEMPLATE_VALUE_STR:  truthy = value->as.str.len > 0; break;
              case SP_TEMPLATE_VALUE_LIST: truthy = !sp_da_empty(value->as.list); break;
            }
          }
          if (dir.negate) truthy = !truthy;
        }

        bool body_end = false;
        bool body_else = false;
        err = sp_template_render_block(p, truthy ? scope : SP_NULLPTR, truthy ? io : SP_NULLPTR, reg, depth, &body_end, &body_else);
        if (err) return err;

        if (body_else) {
          err = sp_template_render_block(p, truthy ? SP_NULLPTR : scope, truthy ? SP_NULLPTR : io, reg, depth, &body_end, &body_else);
          if (err) return err;
          if (body_else) return SP_TEMPLATE_ERR_UNEXPECTED_ELSE;
        }

        if (!body_end) return SP_TEMPLATE_ERR_UNTERMINATED_IF;
        break;
      }

      case SP_TEMPLATE_DIR_ELSE: {
        *hit_else = true;
        return SP_TEMPLATE_OK;
      }
...
sp_template_err_t sp_template_render(sp_io_writer_t* io, sp_str_t tmpl, sp_template_scope_t* scope, sp_template_registry_t* reg) {
  ...
  bool hit_end = false;
  bool hit_else = false;
  sp_template_err_t err = sp_template_render_block(&parser, scope, io, reg, 0, &hit_end, &hit_else);
  if (err) return err;
  if (hit_else) return SP_TEMPLATE_ERR_UNEXPECTED_ELSE;
  if (hit_end) return SP_TEMPLATE_ERR_UNEXPECTED_END;
  return SP_TEMPLATE_OK;
}

#endif // SP_TEMPLATE_H_IMPL
